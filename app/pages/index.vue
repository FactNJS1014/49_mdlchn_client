                ...
                <template #default>
                    <v-tabs-window v-model="tab">
                        ...
                        <v-tabs-window-item value="one">
                            ...
                            <v-form @submit.prevent="handleSubmitForm">
                                <div class="flex gap-2 align-center">
                                    <h1>Employee ID :</h1> <span> <input type="text"
                                            class="w-full px-3 py-2 border border-gray-800 rounded-lg focus:outline-none"
                                            v-model="empno" required> </span>
                                </div> <v-row no-gutters class="gap-3"> <v-col cols="4">
                                        <div class="mt-3 font-semibold">Line No. :</div> <v-select v-model="line"
                                            variant="outlined" rounded="md" :items="line_list" item-title="LINE_NAME"
                                            :rules="LineRules"> </v-select>
                                    </v-col> <v-col cols="4">
                                        <div class="mt-3 font-semibold ">Issue No. :</div> <v-text-field
                                            variant="outlined" rounded="md" v-model="issue_no" :rules="IssueNoRules">
                                        </v-text-field>
                                    </v-col> <v-col cols="3" md="3">
                                        <div class="mt-3 font-semibold ">Issue Date :</div> <input type="date"
                                            ...
                                            v-model="date_rec" required readonly />
                                    </v-col> </v-row> <v-row no-gutters>
                                    <div class="mt-3 font-semibold ">Status :</div> <span class="mt-1 "> <v-radio-group
                                            inline class="gap-4" v-model="status" :rules="StatusRules"> <v-radio
                                                label="Urgent" value="Urgent" color="info"></v-radio> <v-radio
                                                label="Normal" value="Normal" color="info"></v-radio> <v-radio
                                                label="New line" value="New line" color="info"></v-radio> <v-radio
                                                label="New model" value="New model" color="info"></v-radio> <v-radio
                                                label="Over Plan" value="Over Plan" color="info"></v-radio>
                                        </v-radio-group> </span>
                                </v-row> <v-row no-gutters class="gap-3"> <v-col cols="4" md="5">
                                        <div class="mt-3 font-semibold ">WO# (เดิม):</div> <v-autocomplete
                                            variant="outlined" rounded="md" :items="won_list" item-title="WON"
                                            item-value="WON" v-model="won_cur" :rules="WorkOrderRules"
                                            @update:modelValue="SelectWonCur"></v-autocomplete>
                                    </v-col> <v-col cols="6" md="5">
                                        <div class="mt-3 font-semibold ">Current Model Code :</div> <v-text-field
                                            variant="outlined" rounded="md" v-model="cur_modelname"
                                            :rules="CurrentModelRules"> </v-text-field>
                                        ...
                                        <div class="mt-3 font-semibold ">Lot Size เดิม:</div> <v-number-input
                                            variant="outlined" rounded="md" v-model="lots" :min="0"
                                            :rules="LotsRules" />
                                    </v-col> <v-col cols="6" md="3" v-if="prs_check">
                                        <div class="mt-3 font-semibold ">Process: (RF) เดิม</div> <v-radio-group inline
                                            class="gap-4" v-model="procs_rf" :rules="ProcessRules"> <v-radio label="CP"
                                                value="CP" color="info"></v-radio> <v-radio label="RF1" value="RF1"
                                                color="info"></v-radio> <v-radio label="RF2" value="RF2"
                                                color="info"></v-radio> </v-radio-group>
                                    </v-col> </v-row> <v-row no-gutters class="gap-3"> <v-col cols="4" md="5">
                                        <div class="mt-3 font-semibold ">WO# (เปลี่ยน):</div> <v-autocomplete
                                            variant="outlined" rounded="md" :items="won_list" item-title="WON"
                                            item-value="WON" v-model="won_chn" :rules="WorkOrderRules"
                                            @update:modelValue="SelectWonChn"></v-autocomplete>
                                    </v-col> <v-col cols="6" md="5">
                                        <div class="mt-3 font-semibold ">Change Model Code :</div> <v-text-field
                                            variant="outlined" rounded="md" v-model="chn_modelname"
                                        ...
                                        <div class="mt-3 font-semibold ">Lot Size เปลี่ยน:</div> <v-number-input
                                            variant="outlined" rounded="md" v-model="lots_chn" :min="0"
                                            :rules="LotsRules" />
                                    </v-col> <v-col cols="6" md="3" v-if="prs_check">
                                        <div class="mt-3 font-semibold ">Process: (RF) เปลี่ยน</div> <v-radio-group
                                            inline class="gap-4" v-model="procs_rf_chn" :rules="ProcessRules"> <v-radio
                                                label="CP" value="CP" color="info"></v-radio> <v-radio label="RF1"
                                                value="RF1" color="info"></v-radio> <v-radio label="RF2" value="RF2"
                                                color="info"></v-radio> </v-radio-group>
                                    </v-col> </v-row> <v-row no-gutters class="gap-6"> </v-row> <v-row no-gutters
                                    class="gap-3"> <v-col cols="4" md="5">
                                        <div class="mt-3 font-semibold ">ECN Control :</div> <v-radio-group inline
                                            class="gap-4" v-model="ecn" :rules="ecnContolRules"> <v-radio
                                                label="No Have" value="No Have" color="info"></v-radio> <v-radio
                                                label="Have" value="Have" color="info"></v-radio> </v-radio-group>
                                    </v-col> <v-col cols="4" md="3">
                                        <div class="mt-3 font-semibold " v-if="have_ecn">ECN Number :</div>
                                        <v-text-field v-if="have_ecn" variant="outlined" density="compact"
                                            v-model="ecn_num" />
                                    </v-col> <v-col cols="4" md="3">
                                        <div class="mt-3 font-semibold " v-if="have_ecn">REV :</div> <v-text-field
                                            v-if="have_ecn" variant="outlined" density="compact" v-model="ecn_rev" />
                                    </v-col> </v-row> <v-row no-gutters class="gap-3"> <v-col cols="6" md="5">
                                        <div class="mt-3 font-semibold ">Program Name :</div> <v-text-field
                                            variant="outlined" density="compact" v-model="prog_name"
                        ...
                        <v-tabs-window-item value="two">
                            ...
                            <v-divider></v-divider>
                            <v-data-table-virtual :headers="headers" :items="get_rec" v-model:search="search"
                                :filter-keys="['OPR_HREC_WON_CHANGE']" density="compact" height="550" fixed-header>
                ...
                <table class="w-full mt-3 border border-gray-600">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="px-2 py-1 bg-red-200 border">{{ details.OPR_HREC_WON_CURRENT }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td class="px-2 py-1 bg-green-200 border">{{ details.OPR_HREC_WON_CHANGE }}</td>
                        </tr>
                        <tr>
                            <th class="px-2 py-1 text-left border">Model ใหม่:</th>
                            <td class="px-2 py-1 bg-green-200 border">{{ details.OPR_HREC_CHNMDLNM }}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td class="px-2 py-1 bg-green-200 border ">{{ details.OPR_HREC_LOTS_CHN }}</td>
                        </tr>
                        <tr>
...
const handleSubmitForm = async () => {
    if (prs.value === 'CP') {
        try {
            ...
            if (id.value === "") {
                const insert = await axios.post('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/oprform', payload)
                ...
                const update = await axios.put('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/oprform/update/' + id.value, payload)
        ...
        try {
            ...
            if (id.value === "") {
                ...
                const insert_rf = await axios.post('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/oprform/rf', payload)
                ...
                const update_rf = await axios.put('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/oprform/rf/update/' + id.value, payload)
...
const DeleteData = async (id: string) => {
    ...
    try {
        ...
        if (button.isConfirmed) {
            const res = await axios.delete('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/data/delete/' + id)
...
const SelectWonCur = async (value: string) => {
    ...
    try {
        const res = await axios.get('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/get/checkmodel', {
...
const fetchApiWon = async () => {
    try {
        const res = await axios.get('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/get/won')
...
const fetchApiModelName = async () => {
    try {
        const res = await axios.get('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/get/modelname')
...
const fetchApiLine = async () => {
    try {
        const res = await axios.get('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/get/line')
...
const GetRecordForm = async () => {
    try {
        const res = await axios.get('http://172.22.64.11/49_modelchange/49_mdlchn_api/api/get/record/opr')
